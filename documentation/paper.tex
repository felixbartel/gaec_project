...
Undoubtedly, the neural network architecture has been the star of the machine learning resurgence of the past few years. It's provable quality of being an universal function approximator makes it a sensible choice for various applications. The main algorithm used for training, backpropagation however is subject to an array of criticism from various authors \cite{zhang2007hybrid, riedmiller1993direct}, and there are various attempts at improving or replacing it \cite{montana1989training}.

A big advantage of the neuroevolution scheme is that for a series of different inputs a single scalar indicator of performance is sufficient in order to optimize a neural network for a certain task.
...
This implementation of the idea from \cite{Sims94evolving3d} uses two species (named left and right) that are always evaluated against the other. We evaluated
...
In this scenario \ref{fig:self_adapt_1_self_training_2_oldrun} can be seen battling for being the better species in the beginning, but then this seems to converge. A quick manual check confirms however, that they are both still actively learning. When left alone to train for 8000 generations (figure \ref{fig:self_adapt1_self_training_2}) we get the best results so far (figure \ref{fig:self_training_2}). The only bot capable of matching this network is the strongest \texttt{reduced} bot, but it never trained against any of the built-in bots directly. (In fact, this network was too strong even for us humans to beat in the game.)
